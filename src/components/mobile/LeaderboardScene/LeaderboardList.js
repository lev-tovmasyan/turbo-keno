...
function LeaderboardList({getGameLeaderboard, userId, gameLeaderboard}) {
    const [isTicketVisible, setIsTicketVisible] = useState(false);
    const [newData, setNewData] = useState(true)
    let [page, setPage] = useState(0)

    const mobileHeight = document.querySelector(`.game-grid`).clientHeight
    const visibleHeight = mobileHeight - (isTicketVisible ? 175 : 214)
    const {list, playersCurrentPlace, isFinished} = gameLeaderboard;
    const {playerId, place, points, possibleWin, ticketsCount} = playersCurrentPlace;
    ...
    useEffect(() => {
        ...
        if (elem) {
            ...
            const scrollEventListener = () => {
                if (elem.scrollTop + elem.clientHeight > elemHeight - 200 && !count) {
                    count++;
                    setPage(page += 1);
                    try {
                        (async () => {
...
const mapStateToProps = ({userReducer: {userId}, gameLeaderboard}) => ({userId, gameLeaderboard});
